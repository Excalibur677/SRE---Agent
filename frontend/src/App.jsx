...
import {
  ...
  Eye, Search, BookOpen, Lightbulb, Play, Square, Clock, Repeat2,
...
// ── Step config ──────────────────────────────────────────────────────────
const STEP_CONFIG = [
  { name: "OBSERVE",   icon: Eye,        color: "text-cyan-400",    bg: "bg-cyan-500/10",    border: "border-cyan-500/30",    label: "Querying Prometheus"        },
  { name: "HUNT",      icon: Search,     color: "text-amber-400",   bg: "bg-amber-500/10",   border: "border-amber-500/30",   label: "Scanning Logs"              },
  { name: "DIAGNOSE",  icon: BookOpen,   color: "text-violet-400",  bg: "bg-violet-500/10",  border: "border-violet-500/30",  label: "Checking Memory"            },
...
// ── Helpers ───────────────────────────────────────────────────────────────
function CineImg({ src, className = "" }) {
...
const ChartTip = ({ active, payload, label }) => {
  ...
  return (
    <div className="liquid-glass rounded-xl px-3 py-2 text-xs text-white">
  ...
  return (
    <div className="relative flex items-center justify-center w-28 h-28">
      <svg className="-rotate-90 w-28 h-28" viewBox="0 0 96 96">
        ...
        <circle cx="48" cy="48" r={r} strokeWidth="5" fill="none" stroke={color}
      ...
      <div className="absolute text-center">
        ...
        <p className="text-[9px] text-white/40 uppercase tracking-widest">conf.</p>
      </div>
    </div>
  );
}
...
function SourceBadge({ source }) {
  const cfg = {
    cloudflare: "text-orange-400 bg-orange-500/10 border-orange-500/20",
  ...
  return <span className={`text-[10px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-full border ${cfg}`}>{source}</span>;
}
...
function StepCard({ step, index, isActive }) {
  ...
  if (!cfg) return null;
  const Icon = cfg.icon;
  const statusColor = step?.status === "done" ? "text-emerald-400" : step?.status === "running" ? cfg.color : "text-white/20";

  return (
    <div className={`liquid-glass rounded-2xl p-4 border transition-all duration-500
      ${step?.status === "running" ? `${cfg.border} shadow-lg` : "border-white/[0.06]"}
      ${step?.status === "done" ? "border-emerald-500/20" : ""}
      ${!step ? "opacity-30" : "opacity-100"}`}>
      <div className="flex items-start gap-3">
        <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${step?.status === "running" ? cfg.bg : step?.status === "done" ? "bg-emerald-500/10" : "bg-white/5"}`}>
          {step?.status === "running" ? (
            <svg className={`w-4 h-4 animate-spin ${cfg.color}`} viewBox="0 0 24 24" fill="none">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-[10px] font-bold uppercase tracking-widest ${statusColor}`}>
              Step {index + 1} — {cfg.name}
            </span>
            {step?.status === "running" && (
              <span className="flex gap-0.5">
                {[0,1,2].map(i => <span key={i} className={`w-1 h-1 rounded-full ${cfg.color.replace("text-","bg-")} animate-bounce`} style={{ animationDelay: `${i*150}ms` }} />)}
              </span>
            )}
          </div>
          <p className="text-xs text-white/50 leading-relaxed">
            {step?.detail ?? cfg.label}
          </p>
          {step?.status === "done" && step?.result && (
            <div className="mt-2 rounded-lg bg-white/[0.03] border border-white/[0.06] px-3 py-2">
              <p className="text-[10px] font-mono text-white/40 truncate">
                {typeof step.result === "string" ? step.result : JSON.stringify(step.result).slice(0, 120) + "…"}
              </p>
            </div>
          )}
        </div>
...
// ── MEMORY CARD ───────────────────────────────────────────────────────────
function MemoryCard({ item, onRerun }) {
  return (
    <div className="liquid-glass rounded-2xl p-4 border border-white/[0.06] hover:border-white/20 transition-all duration-200 group">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {item.isRecurring && (
            <span className="flex items-center gap-1 text-[10px] text-red-400 bg-red-500/10 border border-red-500/20 rounded-full px-2 py-0.5">
              <Repeat2 className="w-2.5 h-2.5" /> Recurring
            </span>
          )}
        </div>
      ...
      <p className="text-[10px] text-white/40 leading-relaxed mb-3 line-clamp-2">{item.root_cause}</p>
      <div className="flex items-center justify-between">
        <span className="text-[10px] text-white/30 font-mono">{item.service}</span>
        <span className={`text-[10px] font-bold ${parseInt(item.accuracy) >= 85 ? "text-emerald-400" : "text-amber-400"}`}>
          {item.accuracy} confidence
        </span>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [activeTab, setActiveTab]       = useState("analysis"); // "analysis" | "memory" | "agent"
  ...
  useEffect(() => {
    fetch(`${API}/api/incidents`).then(r => r.json())
      .then(d => setMockList(d.data ?? []))
      .catch(() => setError("Backend offline"));
  }, []);

  // ── Fetch selected mock incident ───────────────────────────────────────
  useEffect(() => {
    if (mode !== "mock" || !selectedId) return;
    setIncident(null); setAnalysis(null); setSteps([]);
    fetch(`${API}/api/incident/${selectedId}`).then(r => r.json())
      .then(d => setIncident(d.data)).catch(() => {});
  }, [selectedId, mode]);
  ...
  const fetchLiveIncidents = useCallback(() => {
    ...
    fetch(`${API}/api/real-incidents`).then(r => r.json())
      ...
      .catch(() => setError("Failed to fetch live incidents"))
      .finally(() => setLiveLoading(false));
  }, []);

  useEffect(() => { if (mode === "live") fetchLiveIncidents(); }, [mode]);

  // ── Prometheus metrics every 10s ───────────────────────────────────────
  useEffect(() => {
    const go = () => fetch(`${API}/api/live-metrics`).then(r => r.json())
      .then(d => { if (d.success) setLiveMetrics(d.data); }).catch(() => {});
    ...
    const poll = () => {
      fetch(`${API}/api/agent/status`).then(r => r.json()).then(d => {
        if (d.success) {
          ...
          if (d.data.lastAlert && d.data.lastAlert !== agentAlert) {
            setAgentAlert(d.data.lastAlert);
            setShowAlert(true);
            fetchMemory();
          }
        }
  ...
  const runAnalysis = async () => {
    setAnalyzing(true); setAnalysis(null); setSteps([]); setError(null); setMemoryContext(null);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      setSteps(prev => {
        const next = [...prev];
        next[i] = { step: i + 1, name: STEP_CONFIG[i].name, status: "running", detail: STEP_CONFIG[i].label };
        return next;
      });
    }
    ...
    try {
      ...
      if (d.steps) {
        setSteps(d.steps.map(s => ({ ...s, status: "done" })));
      }
      setAnalysis(d.data);
      ...
      setError(e.message);
      setSteps(prev => prev.map(s => s.status === "running" ? { ...s, status: "done", detail: "Error occurred" } : s));
    } finally {
      setAnalyzing(false);
    }
  };
  ...
  const toggleAgent = async () => {
    const endpoint = agentActive ? "/api/agent/stop" : "/api/agent/start";
  ...
  const clearMemory = async () => {
    ...
    setMemory({ incidents: [], patterns: {} });
  };

  const handleSelect = (item) => {
    setDropOpen(false); setAnalysis(null); setSteps([]); setError(null);
    if (mode === "mock") setSelectedId(item.id);
    else { setLiveIncident(item); setIncident(null); }
  };
  ...
  const activeLabel    = mode === "live"
    ? (liveIncident?.title ?? "Select Live Incident")
    : (mockList.find(i => i.id === selectedId)?.title ?? selectedId);
  ...
  return (
    <div className="bg-black min-h-screen text-white overflow-x-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {showAlert && agentAlert && (
        <div className="fixed top-0 left-0 right-0 z-[100] flex justify-center pt-4 px-4 animate-fadeIn">
          <div className="liquid-glass rounded-2xl border border-red-500/40 bg-red-500/10 px-6 py-4 flex items-center gap-4 max-w-2xl w-full shadow-2xl shadow-red-500/20">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-xs font-bold text-red-400 uppercase tracking-widest">Agent Auto-Detection</p>
              </div>
              <p className="text-sm text-white font-semibold">Anomaly detected: {agentAlert.anomalies?.join(", ")}</p>
              <p className="text-xs text-white/50 mt-0.5 truncate">Root cause: {agentAlert.conclusion?.root_cause}</p>
            </div>
            <button onClick={() => setShowAlert(false)} className="text-white/30 hover:text-white text-xs shrink-0">✕</button>
          </div>
        </div>
      )}
      ...
      <section className="relative min-h-screen flex flex-col overflow-hidden">
        ...
        <nav className="relative z-20 flex justify-center pt-6 px-6">
          <div className="liquid-glass rounded-full flex items-center gap-1 px-2 py-2 w-full max-w-4xl">
            <div className="flex items-center gap-2 px-3 py-1.5">
              ...
              <span className="text-sm font-semibold text-white">SRE-Pulse <span className="text-violet-400">Agent</span></span>
            </div>
            <div className="flex-1 flex items-center justify-center gap-1">
              {["Active Incidents", "Metrics Engine", "System Logs"].map(l => (
                <button key={l} className="px-4 py-1.5 rounded-full text-xs text-white/60 hover:text-white hover:bg-white/10 transition-all">{l}</button>
              ))}
            </div>
            {/* Agent toggle in nav */}
            <button onClick={toggleAgent}
              className={`flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-semibold transition-all duration-300 border
                ${agentActive
                  ? "bg-emerald-500/20 border-emerald-500/40 text-emerald-400 shadow-lg shadow-emerald-500/20"
                  : "liquid-glass border-white/10 text-white/60 hover:text-white"}`}>
              {agentActive
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-6 pb-20 pt-12">
          {/* Agent status pill */}
          <div className={`liquid-glass rounded-full px-4 py-1.5 text-xs flex items-center gap-2 mb-8 transition-all duration-500
            ${agentActive ? "text-emerald-400 border border-emerald-500/20" : "text-white/60"}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${agentActive ? "bg-emerald-400 animate-pulse" : "bg-red-500 animate-pulse"}`} />
            {agentActive ? `🤖 Agent watching — ${activeIncident?.service ?? "all services"}` : "Agent standby — click Start Agent to activate"}
          </div>

          <h1 className="serif text-5xl sm:text-7xl lg:text-8xl font-normal leading-tight max-w-5xl mb-6">
            Autonomous SRE<br />
            <em className="italic text-white/70">intelligence that never sleeps.</em>
          </h1>
          <p className="text-white/50 text-base max-w-xl mb-10 leading-relaxed">
          ...
          {/* Mode toggle */}
          <div className="liquid-glass rounded-full flex items-center p-1 gap-1 mb-6">
            <button onClick={() => { setMode("mock"); setAnalysis(null); setSteps([]); }}
              className={`rounded-full px-5 py-2 text-xs font-semibold transition-all duration-200 flex items-center gap-2
                ${mode === "mock" ? "bg-white text-black" : "text-white/50 hover:text-white"}`}>
              <Database className="w-3.5 h-3.5" /> Simulation Mode
            </button>
            <button onClick={() => { setMode("live"); setAnalysis(null); setSteps([]); }}
              className={`rounded-full px-5 py-2 text-xs font-semibold transition-all duration-200 flex items-center gap-2
                ${mode === "live" ? "bg-white text-black" : "text-white/50 hover:text-white"}`}>
              <Radio className="w-3.5 h-3.5" /> Live Production Outages
            </button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full max-w-2xl">
            <div className="relative flex-1 w-full">
              <button onClick={() => setDropOpen(o => !o)}
                ...
                {liveLoading
                  ? <RefreshCw className="w-4 h-4 shrink-0 animate-spin text-white/40" />
                  : <ChevronDown className={`w-4 h-4 shrink-0 transition-transform ${dropOpen ? "rotate-180" : ""}`} />}
              </button>
              {dropOpen && (
                <div className="absolute top-full mt-2 left-0 right-0 z-50 liquid-glass rounded-2xl overflow-hidden max-h-72 overflow-y-auto">
                  {mode === "live" && (
                    <div className="px-4 pt-3 pb-2 border-b border-white/[0.06] flex items-center justify-between">
                      <span className="text-[10px] uppercase tracking-widest text-white/30">Live Outages — Cloudflare &amp; GitHub</span>
                      <button onClick={fetchLiveIncidents} className="text-[10px] text-white/40 hover:text-white flex items-center gap-1">
            ...
            <button onClick={runAnalysis} disabled={analyzing || !activeIncident}
              className="rounded-full px-6 py-3 text-sm font-semibold whitespace-nowrap transition-all duration-300 flex items-center gap-2
                         bg-white text-black hover:bg-white/90 disabled:opacity-50 disabled:cursor-wait shadow-[0_0_30px_rgba(255,255,255,0.2)]">
              {analyzing
                ? <><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none">
                    <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" className="opacity-25" />
                    <path d="M4 12a8 8 0 018-8" stroke="currentColor" strokeWidth="3" strokeLinecap="round" />
                  </svg>Agent thinking… {elapsed}s</>
                : <><Sparkles className="w-4 h-4" />Run Agent Analysis</>}
            </button>
          </div>
      ...
      {/* ── PROMETHEUS BANNER ─────────────────────────────────────────── */}
      {liveMetrics && (
        <div className="flex justify-center py-4 px-6 bg-black">
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 2 — METRICS
      ══════════════════════════════════════════════════════ */}
      <section ref={metricsRef} className="relative py-32 px-6"
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="serif text-5xl font-normal">Infrastructure anomalies, <em className="italic text-white/60">visualised.</em></h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { data: dbData,  color: "#6366f1", label: "Active DB Connections" },
              { data: errData, color: "#f43f5e", label: "HTTP 5xx Error Rate (%)" },
                ...
                <ResponsiveContainer width="100%" height={140}>
                  <AreaChart data={data} margin={{ top: 4, right: 4, left: -28, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="value" stroke={color} strokeWidth={2}
                      fill={`url(#g${color})`} dot={false} activeDot={{ r: 4, fill: color, strokeWidth: 0 }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ))}
          ...
          <div className="liquid-glass rounded-3xl p-6 mt-4 flex items-center gap-6">
            ...
            <div className="flex-1">
              <div className="flex justify-between mb-2">
                <span className="text-xs text-white/40 uppercase tracking-wider">CPU Utilization</span>
                <span className="text-sm font-bold text-white">{lastCPU}%</span>
              </div>
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 3 — LOGS
      ══════════════════════════════════════════════════════ */}
      <section ref={logsRef} className="relative py-32 overflow-hidden"
        style={{ opacity: 0, transform: "translateY(24px)", transition: "opacity 0.9s ease, transform 0.9s ease" }}>
        <div className="absolute inset-0">
          <CineImg src={IMGS.logs} className="absolute inset-0" />
          <div className="absolute inset-0 bg-black/80" />
        </div>
        <div className="relative z-10 max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            <p className="text-xs uppercase tracking-widest text-white/30 mb-4">System Intelligence</p>
            <h2 className="serif text-5xl font-normal">Raw signals, <em className="italic text-white/60">decoded by the agent.</em></h2>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6">
            <div className="liquid-glass rounded-3xl overflow-hidden flex flex-col" style={{ height: "480px" }}>
              ...
              <div className="flex-1 overflow-y-auto p-5 font-mono text-[11px] leading-relaxed space-y-1.5">
                ...
                {logs.map((line, i) => {
                  const isErr  = /ERROR|CRITICAL|FATAL/.test(line);
                  const isWarn = /WARN/.test(line);
                  return (
                    <div key={i} className={`flex gap-2.5 rounded-lg px-2 py-1 hover:bg-white/[0.04] transition-colors
                      ${isErr ? "text-red-400 bg-red-500/5" : isWarn ? "text-amber-400" : "text-white/40"}`}>
                      <span className="text-white/20 shrink-0 w-5 text-right select-none">{String(i + 1).padStart(2, "0")}</span>
                  ...
                  {activeList.map(inc => (
                    <button key={inc.id} onClick={() => handleSelect(inc)}
                      className={`w-full text-left rounded-2xl px-4 py-3 transition-all border
                        ${(mode === "mock" ? inc.id === selectedId : inc.id === liveIncident?.id) ? "bg-white/10 border-white/20" : "border-transparent hover:bg-white/5"}`}>
                      {inc.source && <SourceBadge source={inc.source} />}
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 4 — AI AGENT BRAIN (THE SHOWSTOPPER)
      ══════════════════════════════════════════════════════ */}
      <section ref={rcaRef} className="relative py-32 px-6"
        style={{ opacity: 0, transform: "translateY(24px)", transition: "opacity 0.9s ease, transform 0.9s ease" }}>
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: "radial-gradient(ellipse 50% 60% at 80% 50%, rgba(139,92,246,0.12) 0%, transparent 70%)" }} />

        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-xs uppercase tracking-widest text-white/30 mb-4">Autonomous AI Agent</p>
            <h2 className="serif text-5xl font-normal">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-6 items-start">
            ...
            {/* LEFT — Video + Agent Controls */}
            <div className="flex flex-col gap-4">
              <div className="liquid-glass rounded-3xl overflow-hidden relative" style={{ height: "260px" }}>
                ...
                <div className="absolute bottom-6 left-6">
                  ...
                  <p className="serif text-2xl font-normal">Telemetry Analysis</p>
                </div>
              </div>
              ...
              <div className="liquid-glass rounded-3xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <Bot className="w-4 h-4 text-violet-400" />
                    <p className="text-xs uppercase tracking-widest text-white/40">Agent Control</p>
                  </div>
                  <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold border
                    ${agentActive ? "text-emerald-400 bg-emerald-500/10 border-emerald-500/30" : "text-white/30 bg-white/5 border-white/10"}`}>
                ...
                <div className="space-y-3">
                  <button onClick={toggleAgent}
                    className={`w-full rounded-2xl py-3 text-sm font-semibold transition-all duration-300 flex items-center justify-center gap-2 border
                      ${agentActive
                  ...
                  <button onClick={runAnalysis} disabled={analyzing || !activeIncident}
                    className="w-full rounded-2xl py-3 text-sm font-semibold transition-all liquid-glass border border-white/10 hover:border-white/30 flex items-center justify-center gap-2 disabled:opacity-40">
                    <Sparkles className="w-4 h-4 text-violet-400" />
                    {analyzing ? `Analyzing… ${elapsed}s` : "Run Manual Analysis"}
                  </button>
                </div>
                ...
                {agentActive && (
                  <div className="mt-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/20 p-3">
                    ...
                    <p className="text-[10px] text-white/40">Polling Prometheus every 30s. Will auto-diagnose anomalies and alert you instantly.</p>
                  </div>
                )}
                ...
                {agentAlert && (
                  <div className="mt-3 rounded-2xl bg-red-500/5 border border-red-500/20 p-3">
                    ...
                    <p className="text-[10px] text-white/40 truncate">{agentAlert.anomalies?.join(", ")}</p>
                    <p className="text-[10px] text-white/60 mt-1 font-semibold truncate">{agentAlert.conclusion?.root_cause}</p>
            ...
            {/* RIGHT — Chain of Thought + Results + Tabs */}
            <div className="flex flex-col gap-4">
              {/* Tab switcher */}
              <div className="liquid-glass rounded-full flex items-center p-1 gap-1">
                {[
                  { key: "analysis", icon: Brain,   label: "Chain of Thought" },
                  { key: "memory",   icon: History, label: `Memory (${memory.incidents.length})` },
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="liquid-glass rounded-3xl p-5">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      {analyzing && (
                        <span className="ml-auto text-[10px] text-violet-400 font-mono animate-pulse">{elapsed}s</span>
                      )}
                    </div>
                    ...
                    {steps.length === 0 && !analyzing && (
                      <div className="flex flex-col items-center py-8 gap-3">
                        <div className="w-12 h-12 rounded-2xl bg-violet-500/10 flex items-center justify-center">
                          <Brain className="w-6 h-6 text-violet-400 opacity-50" />
                        </div>
                        <p className="text-xs text-white/30 text-center">Click Run Agent Analysis to watch the AI think step by step</p>
                      </div>
                    )}

                    <div className="space-y-3">
                      {STEP_CONFIG.map((cfg, i) => (
                        <StepCard key={cfg.name} step={steps[i]} index={i} isActive={analyzing} />
                      ))}
                    </div>
                      ...
                      <div className="mt-4">
                        <div className="h-1 rounded-full bg-white/10 overflow-hidden">
                          <div className="h-full rounded-full transition-all duration-700"
                            style={{
                              width: `${(steps.filter(s => s?.status === "done").length / 4) * 100}%`,
                              background: "linear-gradient(90deg, #6366f1, #8b5cf6, #06b6d4)",
                              boxShadow: "0 0 10px rgba(99,102,241,0.5)",
                            }} />
                        </div>
                        <p className="text-[10px] text-white/30 mt-1 text-right">
                          {steps.filter(s => s?.status === "done").length} / 4 steps complete
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      {memoryContext?.isRecurring && (
                        <div className="liquid-glass rounded-2xl p-4 border border-red-500/30 bg-red-500/5 flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-white/40 mt-0.5">This incident pattern has occurred {memoryContext.occurrences} times before</p>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="liquid-glass rounded-3xl p-6 flex items-start gap-5 border border-violet-500/20">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="liquid-glass rounded-2xl p-4">
                            <p className="text-xs text-white/50 leading-relaxed">{analysis.explanation}</p>
                          </div>
                          <div className="flex items-center gap-1.5 mt-3">
                            <Shield className="w-3 h-3 text-emerald-400" />
                            <span className="text-[10px] text-emerald-400">Confidence: {analysis.accuracy}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="liquid-glass rounded-3xl p-6">
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-[10px] uppercase tracking-widest text-white/40">Remediation Command</p>
                          </div>
                  ...
                  <div className="liquid-glass rounded-3xl p-5">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs uppercase tracking-widest text-white/40">Incident Memory</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={fetchMemory} className="text-[10px] text-white/40 hover:text-white flex items-center gap-1">
                          <RefreshCw className="w-3 h-3" />
                        </button>
                        <button onClick={clearMemory} className="text-[10px] text-red-400/60 hover:text-red-400 flex items-center gap-1">
                        ...
                        <div className="space-y-1">
                          {Object.entries(memory.patterns).filter(([,v]) => v > 0).map(([k, v]) => (
                            <div key={k} className="flex items-center justify-between">
                              ...
                              <span className={`text-[10px] font-bold ${v >= 3 ? "text-red-400" : v >= 2 ? "text-amber-400" : "text-white/40"}`}>
                                ×{v}
                              </span>
                            </div>
                          ))}
                    ...
                    <div className="space-y-3 max-h-[500px] overflow-y-auto">
                      {memory.incidents.length === 0 && (
                        <div className="text-center py-8">
                          ...
                          <p className="text-xs text-white/20">No incidents in memory yet</p>
                          <p className="text-[10px] text-white/10 mt-1">Run an analysis to start building memory</p>
                        </div>
                      )}
                      {memory.incidents.map((item) => (
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 5 — THREAT REGISTRY
      ══════════════════════════════════════════════════════ */}
      <section ref={threatRef} className="relative py-32 px-6"
        style={{ opacity: 0, transform: "translateY(24px)", transition: "opacity 0.9s ease, transform 0.9s ease" }}>
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-xs uppercase tracking-widest text-white/30 mb-4">System Health Registry</p>
            <h2 className="serif text-5xl font-normal">Infrastructure layers, <em className="italic text-white/60">monitored relentlessly.</em></h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              { src: IMGS.dbLayer, icon: Database, accent: "text-indigo-400", tag: "Database Layer Health", title: "Connection Pool", subtitle: "State Monitor",
              ...
              { src: IMGS.net, icon: Wifi, accent: "text-violet-400", tag: "Network Layer Latency", title: "API Failure", subtitle: "Rate Tracker",
            ...
            ].map(({ src, icon: Icon, accent, tag, title, subtitle, stats }) => (
              <div key={tag} className="group liquid-glass rounded-3xl overflow-hidden relative" style={{ minHeight: "420px" }}>
                <CineImg src={src} className="absolute inset-0 transition-transform duration-700 group-hover:scale-105" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent" />
                <div className="absolute inset-0 p-8 flex flex-col justify-end">
                  ...
                  <h3 className="serif text-3xl font-normal mb-4">{title}<br /><em className="italic text-white/60">{subtitle}</em></h3>
      ...
      <footer className="border-t border-white/[0.06] py-8 px-6">
        <div className="max-w-6xl mx-auto flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-4 text-xs text-white/30">
            ...
            <span className={`flex items-center gap-1 ${agentActive ? "text-emerald-400" : ""}`}>
              <Bot className="w-3.5 h-3.5" />{agentActive ? "Agent Active" : "Agent Off"}
            </span>
          </div>
        </div>
      </footer>
